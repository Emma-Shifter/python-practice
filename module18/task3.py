...
def get_world_name(path: str) -> str:
    ...
    if (response.status_code == 200): return dict(json.loads(response.text))["name"]
def get_pilot_info(path: str) -> dict:
    response = get_page(path)
    if (response.status_code == 200): pilot_data = dict(json.loads(response.text))
    return {
...
def get_millennium_falcon_info() -> dict:
    ...
    if (response.status_code == 200): shipData = dict(json.loads(response.text))
    return {
        "max_atmosphering_speed": shipData["max_atmosphering_speed"],
        "pilots": [get_pilot_info(pilot_api_page) for pilot_api_page in shipData["pilots"]],
...
def main():
    ...
    print(info)
    with open("shipData.json", "w", encoding="UTF-8") as file:
